...
Data uncompress(Data srcbuf, uint destlen = 0u, int winbits = 15)
{
	int err;
	Data destbuf;
...
class Compress
{
  ...
  public:
	...
	{   int err;
		...
		destbuf = new Data(zs.avail_in + buf.length);
		zs.next_out = cast(ubyte*)destbuf.ptr;
...
class UnCompress
{
  ...
  public:
	...
	{   int err;
		...
		destbuf = new Data(zs.avail_in * 2 + destbufsize);
		zs.next_out = cast(ubyte*) destbuf.ptr;
		zs.avail_out = destbuf.length;
	...
	body
	{
	  ...
	  L1:
		destbuf = new Data(zs.avail_in * 2 + 100);
		zs.next_out = cast(ubyte*) destbuf.ptr;
		zs.avail_out = destbuf.length;
