// Copy of std.socket from Phobos 1.060 with some adjustments
...
SocketException getSocketException(string msg)
{
	return new SocketException(msg, _lasterr());
}

/// Return true if the last socket operation errored because the socket was non-blocking and the operation would have blocked.
bool wouldBlock()
{
	version(Win32)
		return _lasterr() == WSAEWOULDBLOCK;
	else version(Posix)
		return _lasterr() == /*EAGAIN*/11;
	else
		static assert(0);
}
