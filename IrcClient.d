...
private:
	...
	void onReadLine(LineBufferedSocket sender, string line)
	{
		...
		else
		{
			...
			params.length = params.length + 1;
			params[length - 1] = line[colon + 1 .. line.length];
		}
		...
		case "433":     // nickname in use
			...
			else
			{
				...
				if (params[1] == nickname)
					nickname ~= format("%d%d%d", rand() % 10, rand() % 10, rand() % 10);
	...
	{
		command("PING", .toString(getUTCtime() / TicksPerSecond));
	}
